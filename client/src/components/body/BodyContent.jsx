...
export default function BodyContent(props) {
	...
	const ConditionalResult = (props) => {
		...
		if (tableJson.length > 0) {
			return (
				<Grid item xs={12}>
					{props.tab === 'ookla' ? (
						<OoklaTable json={state.tableJson} />
					) : (
						<IperfTable json={state.tableJson} />
					)}
				</Grid>
			);
		}
	...
	const ConditionalLoad = (props) => {
		...
		return props.tab === 'ookla' ? (
			<OoklaForm onClick={onClick} />
		) : (
			<IperfForm onClick={onClick} />
		);
	};
	...
	return (
		<Container fixed>
			...
			>
				<ConnectionSnackbarContent
					message={state.errorSnackbarMessage}
					variant={'error'}
					onClose={closeSnackBar}
				/>
			</Snackbar>
